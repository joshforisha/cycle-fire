...
* `child(path: string)` – Returns a child reference object containing these properties
* `values` – A stream of the ref's values, utilizing [`firebase.database.Reference.on()`](https://firebase.google.com/docs/reference/js/firebase.database.Reference#on) with `"value"`

#### <a id="errors"></a> `errors`

A stream of errors generated by the defined [firebaseActions](#firebaseActions). You should subscribe to this stream and filter error messages by types as outlined in the specific Firebase API calls.
